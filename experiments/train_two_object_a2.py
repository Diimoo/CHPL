...
def main():
    ...
    out_json = out_dir / f'two_object_a2_{run_id}.json'
    out_ckpt = out_dir / f'two_object_a2_{run_id}.pt'
    ...
    data = [(p.image, p.label) for p in pairs56]

    combos = sorted({_parse_label(lbl) for (_img, lbl) in data})
    ...
    for img, label in test_data:
        res = brain.test_binding(torch.from_numpy(img).float(), label)
        if res['same_concept']:
            n_correct += 1
